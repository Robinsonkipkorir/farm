  ...
  <div class="container-fluid">
    ...
    <div class="collapse navbar-collapse" id="myNavbar">
	    <ul class="nav navbar-nav">
	      ...
	      <li class="active1"><a href="chats.php">Chats</a></li>
...
<div id="modules" class="container text-center">
  ...
  <div class="tab-content">
    <div id="chat" class="tab-pane fade in active">      
      <?php 
        $agronomistsUsernameQuery = mysqli_query($connect, "SELECT * FROM agronomists");
        while($agro_row = mysqli_fetch_assoc($agronomistsUsernameQuery))
        { 
          echo "<div class='well text-left' style='background-color: #f5fff2; border-color: #518e40; border-radius: 10px;'>
                <a href='chats_platform.php?begin_chat=".urlencode($agro_row['username'])."'>
                <img src='uploaded_images/".$agro_row['image_upload']."' class='img-circle' width='30px' height='30px'> ". $agro_row['username'] ."</a></div>";
        }  
      ?>            
    </div>
    <div id="profile" class="tab-pane fade">
      <?php
        $farmersNameQuery2 = mysqli_query($connect, "SELECT * FROM farmers WHERE username = '$username'"); 
        $agro_row3 = mysqli_fetch_assoc($farmersNameQuery2);       
      ?>
      <img src='uploaded_images/<?php echo $agro_row3['image_upload']; ?>' class='img-circle'  width='200px' height='200px'>       
      <p><b style="font-size: 120%;">Name:</b> 
        <span class="label label-success" style="font-size: 103%;"><?php 
        
        $farmersNameQuery = mysqli_query($connect, "SELECT * FROM farmers WHERE username = '$username'"); 
        $agro_row2 = mysqli_fetch_assoc($farmersNameQuery);
        echo $agro_row2['name'];
        ?></span>
      </p>
      <p><b style="font-size: 120%;">Natural Region:</b> 
        <span class="label label-success" style="font-size: 103%;"><?php
        echo $agro_row2['region_name'];
        ?></span>        
      </p>
      ...
      <form method="post" action="includes/upload_profile.inc.php" enctype="multipart/form-data">        
