...
<div class="container">				
	<div class="row">
		...
		<div class="col-md-4">			
			<form role="form" id="form1" method="post" action="includes/agronomist_login.inc.php">
				<h4 class="text-center margin-title">AGRONOMIST LOGIN</h4>				
				...
				<div class="form-group">
					...
					<input type="password" name="password" placeholder="Enter Password" class="form-control" pattern="(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{8,}" title="Must contain at least one Number and one Uppercase and Lowercase letter, and at least 8 or more characters" required="required">
				</div>
				<div class="text-left" style="display: inline;">
      				<label><a href="agronomist_forgotpassword.php"> Forgot Password?</a></label> 
    			</div>
    			...
    			<button type="submit" id="btn-login" class="btn btn-default button-right" name="login">Login</button>			
			</form>
			<?php                                            
			        if(!isset($_GET['login'])){
			            exit();
